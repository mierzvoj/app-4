...
def create_room(db: Cursor, owner_id: int, name: str, password: str, ):
    ...
    out = db.execute("INSERT INTO rooms (owner_id, name, password) VALUES (?, ?, ?) RETURNING id",
                     (owner_id, name, hashed_password)).fetchone()[0]
...
def join_room(db: Cursor, user_id: int, room_id: int, password: str) -> bool:
    ...
    out = db.execute("INSERT INTO joined_rooms (user_id, room_id) VALUES (?, ?)", (user_id, room_id))
    return out


def joined_room(db: Cursor, user_id: int, room_id: int) -> bool:
    return len(
        db.execute("SELECT * FROM joined_rooms WHERE room_id = ? AND user_id = ?", (room_id, user_id)).fetchall()) > 0
...
def get_all_rooms(db: Cursor) -> List[Room]:
    return [Room(id=room[0], owner=room[2]) for room in db.execute("SELECT * FROM rooms")]
